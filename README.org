...
**** named reports
     ...
     easier to pull up a report in the future, you can save it as a
     named report.  this can be done by running

     @@html:<kbd>@@M-x@@html:</kbd>@@ ~metrics-tracker-save-named-report~

     or by modifying the variable ~metrics-tracker-named-reports~ in
     ~customize~.  the fields in named reports are the same as the
     input parameters requested during report generation.  this is
     what it looks like in ~customize~.
...
** customization
    ...
    - metrics-tracker-named-reports :: a list of saved reports
